...
// Handle errors in the session store (optional)
...
app.use(
  session({
    secret: 'CMD123', // Replace with a strong secret key for session encryption
    resave: false,
    saveUninitialized: false,
    store: store, // Use the MongoDB session store (not sessionStore)
    cookie: {
...
function formatReport(orders) {
  let formattedReport = `
    ...
    <div class="report-container"> <!-- Added container -->
      <table>
        <tr>
          <th>OrderID</th>
          <th>Username</th>
    ...
    if (items.length === 0) {
      ...
      items.forEach((item, index) => {
        ...
        if (index === 0) {
          ...
          formattedReport += `<td>${order.orderId || ''}</td><td>${order.username || ''}</td><td>${item.itemName || ''}</td><td>${item.quantity || ''}</td><td>Php ${(item.price || 0).toFixed(2)}</td><td>${order.phone || ''}</td><td>${order.location || ''}</td><td>Php ${totalPrice}</td><td>Delivered</td><td>${order.paymentmethod || ''}</td><td>${formattedOrderDate}</td>`;
        } else {
          // For additional items in the same order, leave Delivery Status and Payment Method cells empty.
          formattedReport += `<td>${order.orderId || ''}</td><td>${order.username || ''}</td><td>${item.itemName || ''}</td><td>${item.quantity || ''}</td><td>Php ${(item.price || 0).toFixed(2)}</td><td>${order.phone || ''}</td><td>${order.location || ''}</td><td></td><td></td><td></td><td></td>`;
        }
        formattedReport += '</tr>';
      });
    }
